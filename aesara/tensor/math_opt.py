...
def local_pow_canonicalize(fgraph, node):
    if node.op == tt_pow:
        ...
        if cst == 0:
            return [broadcast_to(1, node.outputs[0].shape, dtype=node.outputs[0].dtype)]
        if cst == 1:
            return [
                broadcast_to(
                    node.inputs[0], node.outputs[0].shape, dtype=node.inputs[0].dtype
                )
            ]
    else:
        return False
...
def local_zero_div(fgraph, node):
    ...
    if isinstance(node.op, Elemwise) and isinstance(
        ...
        if local_mul_canonizer.get_constant(node.inputs[0]) == 0:
            ret = broadcast_to(0, node.outputs[0].shape, dtype=node.outputs[0].dtype)
    ...
    if node.op == mul:
        ...
        for input in node.inputs:
            ...
            if y == 1.0:
                ...
                return [
                    broadcast_to(0, node.outputs[0].shape, dtype=node.outputs[0].dtype)
                ]
            else:
        ...
        if new_inputs != node.inputs:
            if new_inputs:
                ...
                if has_neg:
                    return [
                        broadcast_to(
                            -1, node.outputs[0].shape, dtype=node.outputs[0].dtype
                        )
                    ]
                else:
                    return [
